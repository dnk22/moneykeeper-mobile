...
export class CategoriesLocalData {
  ...
  /**
   * Kiểm tra xem có dữ liệu danh mục trong database không
   * @returns Promise chứa số lượng bản ghi hoặc undefined nếu có lỗi
   */
  public async isCategoryDataExist() {
    try {
      return await database.read(async () => {
        return await this.categoriesCollection.query().fetchCount();
      });
    } catch (error) {
  ...
  /**
   * Import dữ liệu danh mục mặc định
   * @returns Promise chứa kết quả import
   */
  public async importDefaultCategories() {
    try {
      const isHaveDataInit = await this.isCategoryDataExist();
      ...
      return await database.write(async () => {
        ...
        const res = await this.categoriesCollection
          .query(
            Q.unsafeSqlQuery(
      ...
      this.throwError('IMPORT-DEFAULT', 'Failed to import default categories');
    }
  }
