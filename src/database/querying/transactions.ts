...
export class TransactionLocalDataSource {
   ...
   * LƯU Ý: Logic cập nhật useCount trong category sẽ được xử lý ở lớp trên (TransactionService).
   */
  /**
   ...
   * 3. Lưu ý: Việc cập nhật useCount của category sẽ được xử lý ở lớp TransactionService
   */
  public async addNewTransaction(transactionData: TTransactions): Promise<TransactionModel> {
    return await database.write(async () => {
      const newTransaction = await this.transactionsCollection.create((item) => {
        Object.assign(item, {
          ...transactionData,
          createdAt: Date.now(), // Thêm timestamp tạo
          updatedAt: Date.now(), // Thêm timestamp cập nhật
        });
      });
  ...
  public async updateTransaction({ id, data }: { id: string; data: TTransactions }): Promise<{
    isUpdateCountCategory: boolean;
    isUpdateBalance: boolean;
    prevAccountId: string;
    prevToAccountId: string | null;
    prevDateTimeAt: number;
    ...
    try {
      return await database.write(async () => {
        ...
        /** Cập nhật dữ liệu */
        await transactionToUpdate.update((item) => {
          Object.assign(item, {
            ...data,
            updatedAt: Date.now(), // Cập nhật timestamp
          });
        });

        /** Xác định các cờ cần thiết cho lớp trên */
        ...
        return {
          ...
          prevToAccountId,
          prevDateTimeAt,
          transactionUpdated: transactionToUpdate,
        };
      });
    } catch (error) {
